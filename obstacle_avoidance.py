...
def  obstacle_avoid_left_right():
    ...
    if (obstacle.is_close(multiranger.left) & (not (from_right or from_front or from_back)) & case==Drone.state_zigzag['left'] ):
        print('state =1 left') 
    ...
    if (obstacle.is_close(multiranger.right)  & (not (from_left or from_front or from_back)) & case==Drone.state_zigzag['right']):
        print('state =1 right') 
    ...
    if (state == 2): #state 2
        ...
        if(obstacle_at_back):
            if (first_detection):
                if (not(obstacle.is_close(multiranger.back))): 
                    no_detection = no_detection + 1
                    if (no_detection >= 2): # for safety
                        state = 3
                        mc.move_distance(0, dist, 0, VELOCITY)
            if (obstacle.is_close(multiranger.back)):
                first_detection =1
                velocity_front = 0.05
        if(obstacle_at_front):
            if (first_detection):
                if (not(obstacle.is_close(multiranger.front))): 
                    no_detection = no_detection + 1
                    if (no_detection >= 2): # for safety
                        state = 3
                        mc.move_distance(0, dist, 0, VELOCITY)
...
def  obstacle_avoid_front_back():
    ...
    if (is_close(multiranger.front) & (not from_back) & (case==Drone.state_zigzag['forward1'] or case==Drone.state_zigzag['start'] or case==Drone.state_zigzag['forward2'] )): 
        print('state =1 front')
        from_front = 1
        if (state==1) :
    ...
    if (is_close(multiranger.back) & (not from_front) & case==Drone.state_zigzag['arrived']): #jamais pour le moment
        print('state =1 back')
        from_back = 1
        if (state==1) :
            pos_estimate_before_y = Drone.est_y #y pos left/right
